...
function renderCoordList() {
  ...
  const myProjects = Object.values(ALL_PROJECTS).filter(p => {
    if (isSuper) return true;   // super admin sees every project
  ...
  let filtered = myProjects.filter(p => {
    ...
    if (S.coordFilterOwner && S.coordFilterOwner !== "all" && ((pr.coordinator || "") !== S.coordFilterOwner)) return false;
  ...
  return `
  <div class="cbar">
    <div class="clabel">⚙ ${(CURRENT_USER&&CURRENT_USER.role==="super_admin") ? "All Projects – Coordinator" : (S.coordName ? esc(S.coordName) + " – Coordinator" : "Coordinator Mode")}</div>
  ...
  <div class="coord-search-bar">
    ...
    ${isSuper ? `<select class="coord-filter-sel" onchange="S.coordFilterOwner=this.value;render()" title="Filter by owner / coordinator">
      ...
      ${Array.from(new Set(Object.values(ALL_PROJECTS).map(p => (p.project&&p.project.coordinator)||"").filter(Boolean))).sort((a,b)=>a.localeCompare(b)).map(o =>
        `<option value="${esc(o)}" ${S.coordFilterOwner === o ? "selected" : ""}>${esc(o)}</option>`).join("")}
    </select>` : ""}
