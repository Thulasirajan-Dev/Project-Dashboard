...
async function submitAddUser() {
  ...
  const dup = Object.values(AS.users).find(u => u.name.toLowerCase() === name.toLowerCase());
  ...
  if (ok) {
    ...
    AS.saving = false; AS.err = "Failed to save. Check Firebase connection."; renderAuth();
  }
}
  ...
  return `<div class="auth-hdr">
    ...
    <div style="text-align:right;flex-shrink:0">
      ...
      <button class="btn btn-sm" style="background:rgba(255,255,255,0.1);color:#ddd;margin-top:6px;font-size:11px"
        onclick="clearSession();window.location.href='/auth/'">Logout</button>
    </div>
  </div>`;
}
