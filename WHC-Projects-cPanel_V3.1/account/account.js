...
async function loadAll() {
  ...
  try {
    ALL_PROJECTS = (await fbGet(coPath("projects"))) || {};
  } catch (e) {
    ALL_PROJECTS = {};
  }
  S.mode = "admin";
  try { render(); }
    ...
    document.getElementById("app").innerHTML =
      `<div style="padding:40px;text-align:center;color:#a33"><b>Something went wrong loading this view.</b><br>
       <span style="font-size:12px;color:#888">${(e && e.message) ? String(e.message).replace(/[<>]/g,"") : "Unknown error"}</span></div>`;
      ...
      <div class="admin-popup-body">
        ${pop.projects && pop.projects.length ? pop.projects.map(p => {
          ...
          return `<div class="admin-popup-proj-row" onclick="closeAdminPopup();openProject('${p.id}')">
            <div style="flex:1">
              <div style="font-size:13px;font-weight:600;color:#1a1a1a;margin-bottom:3px">${esc(projTitle(p)||"Unnamed")}</div>
...
async function saveMilestoneNote(projectId, gi, mi, note) {
  const proj = ALL_PROJECTS[projectId];
  if (!proj || !proj.quotationGroups || !proj.quotationGroups[gi]) return;
  const ms = proj.quotationGroups[gi].milestones && proj.quotationGroups[gi].milestones[mi];
  if (!ms) return;
  ms.followupNote = note;
  try { await fbSet(coPath("projects/" + projectId + "/quotationGroups"), proj.quotationGroups); } catch (e) {}
}
...
async function creditGroupMilestone(projectId, gi, mi) {
  const proj = ALL_PROJECTS[projectId];
  if (!proj || !proj.quotationGroups || !proj.quotationGroups[gi]) return;
  const ms = proj.quotationGroups[gi].milestones && proj.quotationGroups[gi].milestones[mi];
  if (!ms) return;
  if (ms.status === "credited") { render(); return; }
  if (!confirm(`Mark "${ms.name || "this milestone"}" as credited? This records the payment as received.`)) return;

  const who = (CURRENT_USER || getSession() || {});
  ms.status = "credited";
  ms.creditedDate = new Date().toISOString();
  ms.creditedBy = who.email || who.name || "";

  // Persist the whole quotationGroups array for this project.
  const ok = await fbSet(coPath("projects/" + projectId + "/quotationGroups"), proj.quotationGroups);
  if (ok) {
    if (typeof logActivity === "function") {
      await logActivity("Account", "Credited milestone",
        (proj.project && proj.project.title) || projectId,
        (ms.name || "") + " · " + (fmtAED(ms.amount || 0)), null, projectId);
    }
  } else {
    // roll back the in-memory change if the save failed
    ms.status = "pending"; ms.creditedDate = ""; ms.creditedBy = "";
    ...
    (p.quotationGroups || []).forEach((g, gi) => {
      ...
      (g.milestones || []).forEach((m, mi) => {
        rows.push({ p, g, gi, mi, l: Object.assign({}, m,
          { amount: m.amount || Math.round(total * (Number(m.pct) || 0) / 100) }) });
      });
    });
  });
  return rows;
...
// Awaiting = team-wide milestones marked Done (stageStatus) but not yet
// credited. Same count for everyone.
function _isAwaiting(l) {
  return (l.stageStatus === "Done") && ((l.status || "pending") !== "credited");
}
...
function renderAdmin() {
  ...
  <div class="cbar">
    ...
    <div style="font-size:11px;color:#8fa3bf">Live data · All projects &amp; milestones</div>
  </div>
  <div class="admin-nav-tabs">
    ...
    ${(CURRENT_USER&&CURRENT_USER.role==="super_admin") ? `<div class="admin-nav-tab ${S.adminTab==="activity"?"on":""}"  onclick="S.adminTab='activity';loadActivity()">🕘 Activity Log</div>` : ""}
  </div>`;
          ...
          <div style="display:flex;justify-content:space-between;align-items:center;gap:12px">
            <div style="flex:1;min-width:0;cursor:pointer" onclick="openProject('${r.p.id}','lpo')">
              ...
              <div style="font-size:12px;color:#888;margin-top:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">
                ...
                · <span style="color:#5b3df5">👤 ${esc(ownerLbl)}</span>${r.g&&r.g.quotationNo?` · <span style="color:#999">Qtn ${esc(r.g.quotationNo)}</span>`:""}
              </div>
            </div>
            <div style="text-align:right;flex-shrink:0">
              <div style="font-weight:700;color:#0d2137;font-size:14px">${fmtAED(r.l.amount||0)}</div>
              <span style="font-size:10px;font-weight:600;padding:2px 8px;border-radius:10px;background:#fdf3df;color:#8a5a00;white-space:nowrap">⏳ Needs credit</span>
            </div>
            <div style="color:#c9a752;font-size:18px;flex-shrink:0">›</div>
          </div>
          <div style="margin-top:8px;background:#fafbfc;border-radius:6px;padding:6px 8px">
            <div style="font-size:10px;color:#aaa;font-weight:600;margin-bottom:2px">📣 Follow-up thread</div>
  ...
  // ── My Milestones tab (cross-project, owned by me) ────────────
    ...
    h += `<div class="admin-body">
      <div class="dash-card" style="margin-bottom:12px">
        <div class="dash-card-title">💰 My Milestones — ${esc(myName || "Account user")} <span style="font-size:11px;color:#aaa;font-weight:400">pending only</span></div>
    ...
    if (!shown.length) {
      ...
      shown.forEach(r => {
        ...
        h += `<div class="attn-row" style="cursor:pointer" onclick="openProject('${r.p.id}','lpo')">
          <div style="flex:1">
            <div class="attn-stage">${esc(r.l.name || "Milestone")} · <span style="color:#888;font-weight:400">${esc(projTitle(r.p) || r.p.id)}</span></div>
            <div style="font-size:11px;color:#999">${esc(pr.client||"")}${pr.coordinator?" · Coordinator: "+esc(pr.coordinator):""}</div>
          </div>
          <div style="text-align:right">
            <div style="font-weight:700;color:#0d2137">${fmtAED(r.l.amount||0)}</div>
            <span style="font-size:10px;font-weight:600;padding:2px 8px;border-radius:10px;background:#fdf3df;color:#8a5a00">⏳ Awaiting credit</span>
          </div>
        </div>`;
      });
      h += `</div></div>`;
  ...
  // ── All Records tab ───────────────────────────────────────
    ...
    <div class="proj-table">
      ...
      ${filtered.map(p => {
        ...
        return `<div class="proj-row" onclick="openProject('${p.id}','scope')">
          ...
          <div class="proj-row-btns" onclick="event.stopPropagation()">
            ...
            <button class="btn btn-sm btn-gold" onclick="openProject('${p.id}','scope')">Edit Project</button>
    ...
    <div style="padding:8px 18px">
      ...
      ${filtered.length ? `<div style="font-family:ui-monospace,'SF Mono',Menlo,Consolas,monospace;font-size:12px;border:1px solid #e5e5e5;border-radius:6px;overflow:hidden">
        ${filtered.map((r,idx)=>{
          const ts = fmtLogTime(r.at);
          const head = `<span style="color:#888">${esc(ts)}</span> <span style="color:#1a3a5c;font-weight:700">${esc(r.byName||resolveUserName(r.by)||"—")}</span> <span style="color:#333">${esc(r.action||"")}</span>${r.target?` <span style="color:#5b3df5">${esc(r.target)}</span>`:""}${r.detail?` <span style="color:#888">— ${esc(r.detail)}</span>`:""}`;
          let changesHtml = "";
          ...
          return `<div style="padding:6px 12px;${idx%2?'background:#fafafa':'background:#fff'};border-bottom:1px solid #f2f2f2">
            <div style="display:flex;gap:8px"><span style="color:#bbb;flex-shrink:0">[${esc(r.module||"-")}]</span><div style="flex:1">${head}${changesHtml}</div></div>
          </div>`;
        }).join("")}
      </div>` : ""}
    </div>`;
  ...
  h += `<div class="footer">${esc(getCompany().name)} &nbsp;·&nbsp; Admin Dashboard &nbsp;·&nbsp; <a href="#" style="color:#888" onclick="event.preventDefault();serverLogout().then(()=>window.location.href='/auth/')">Logout</a></div>`;
  return h;
}
