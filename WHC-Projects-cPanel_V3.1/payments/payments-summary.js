// ============================================================
//  Winner Holistic Consultants – Milestone Dashboard
//  payments/payments-summary.js
//  Standalone module: project-by-project milestone collection details.
//  Reads projects + their quotationGroups[].milestones.
// ============================================================

// Flatten a project's milestones (across all quotation groups) to a list with
// a computed amount and normalized fields, then total them.
function _projMilestones(p) {
  const out = [];
...
function _milestoneTotals(list) {
  const t = { raised: 0, credited: 0, pending: 0, count: 0, creditedCount: 0 };
  list.forEach(l => {
    const amt = Number(l.amount) || 0;
    t.raised += amt; t.count++;
    if (l.status === "credited") { t.credited += amt; t.creditedCount++; }
    else t.pending += amt;
  });
  return t;
}
...
async function loadAndRenderPayments() {
  ...
  const rows = Object.values(projects).map(p => {
    const lpos = _projMilestones(p);
    const t = _milestoneTotals(lpos);
    ...
    return {
      id: p.id,
      title: projTitle(p) || p.id || "Untitled",
...
function renderPayments() {
  ...
  <div class="pbar-header">
    <div class="pbar-label">📊 Milestone Dashboard</div>
    <button data-vo-safe class="btn btn-sm btn-gold keep-mobile" onclick="loadAndRenderPayments()">${svgIcon('refresh',14,'#fff')} Refresh</button>
  </div>
  ...
  <div class="search-bar">
    <input class="search-input" id="pay-search" placeholder="Search project, client, coordinator, quotation…"
      value="${esc(S.search)}" oninput="S.search=this.value;renderPayments();_refocusSearch('pay-search')"/>
  ...
  view.forEach(r => {
    ...
    h += `<div class="pay-card">
      ...
      <div class="pay-figures">
        <div><span class="pay-fl">Raised</span><span class="pay-fv">${fmtAED(r.raised)}</span><span class="pay-fc">${r.count} milestone${r.count!==1?"s":""}</span></div>
      ...
      <div class="pay-card-foot">
        <a href="/coordinator/?id=${encodeURIComponent(r.id)}&tab=lpo" class="pay-open" title="Open project milestones">Open milestones →</a>
      </div>
    </div>`;
  });
